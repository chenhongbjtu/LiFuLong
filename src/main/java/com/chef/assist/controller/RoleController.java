...
public class RoleController {
    ...
    public void initRolesAndAdmin() throws InvalidKeySpecException, NoSuchAlgorithmException {
        Role roleAdmin = roleMapper.findByRoleName("超级管理员");
        Long adminId = null;
        if(roleAdmin==null){
            roleAdmin = new Role();
        ...
        Role roleChef = roleMapper.findByRoleName("仓库管理员");
        if(roleChef==null){
            roleChef = new Role();
            roleChef.setRoleName("仓库管理员");
            roleMapper.insert(roleChef);
        }

        Role roleWaiter = roleMapper.findByRoleName("订单管理员");
        if(roleWaiter==null){
            roleWaiter = new Role();
            roleWaiter.setRoleName("订单管理员");
